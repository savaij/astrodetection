...
def create_index_cosine(df_embeddings: pd.DataFrame):
    """
    Costruisce un indice appoggiandosi a scikit-learn.
    Restituisce un dizionario con:
        - 'model'      : NearestNeighbors addestrato (metric='cosine')
        - 'embeddings' : matrice normalizzata delle embedding
        - 'ids'        : array degli indici originali (stringhe)
    """
    # normalizzazione L2 necessaria per la distanza coseno
    emb = normalize(df_embeddings.to_numpy().astype("float32"), axis=1)

    nn = NearestNeighbors(
        metric="cosine",        # distanza coseno = 1 - similarità
        algorithm="brute"       # brute → compatibile ovunque; per dataset grandi valuta 'auto'
    )
    nn.fit(emb)

    return {"model": nn, "embeddings": emb, "ids": df_embeddings.index.to_numpy()}
...
def find_matches(
    ...
    """
    Trova tutte le coppie con similarità coseno ≥ `threshold`
    usando radius_neighbors di scikit-learn.
    """
    # distanza coseno corrispondente alla similarità desiderata
    radius = 1.0 - threshold      # es. threshold=0.7 → radius=0.3
    ...
    for i in trange(0, len(df_embeddings_search), batch_size, disable=not verbose):
        batch = df_embeddings_search.iloc[i : i + batch_size]
        batch_emb = normalize(batch.to_numpy().astype("float32"), axis=1)

        # radius_neighbors restituisce per ogni vettore (dists, idxs)
        dists, neighs = nn.radius_neighbors(batch_emb,
                                            radius=radius,
                                            return_distance=True)

        for j, (dvec, ivec) in enumerate(zip(dists, neighs)):
            source_id = batch.index[j]
            # trasformo le distanze in similarità (1 - distanza)
            sims = 1.0 - dvec
            for tgt_idx, sim in zip(ivec, sims):
                target_id = ids_full[tgt_idx]
                if str(source_id) != str(target_id):           # esclude match con sé stesso
                    list_indices.append([str(source_id), str(target_id), float(sim)])

    # dataframe finale (ed elimina i duplicati A-B / B-A)
